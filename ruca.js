...
async function indexarMongo(dbName, collection) {
  try {
    // const sentenciasDir = path.join(__dirname, 'sentencias');
    const sentenciasDir = 'C:/Users/kathe/Desktop/Leo/sentencias/saludCA';
...
async function indexarBase(dbName, collection, namespace, pathSentence) {
  try {
    const sentenciasDir = pathSentence;
    ...
    for (const archivo of archivos) {
      try {
        ...
        await IndexarPinecone2.create(jsonData.texto_sentencia, 'bigdata', namespace, metadata);
        console.log(`✅ Sentencia ${archivo} indexada correctamente`);
...
// indexarBase('buscadorDB', 'jurisprudencia', 'jurisprudencia', 'C:/Users/kathe/Desktop/Leo/sentencias/cobranza');