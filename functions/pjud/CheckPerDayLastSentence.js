...
const { basesDic, refererDic, baseArray } = require('./constantsPerDay');

const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));

class CheckPerDayLastSentence {
    ...
    static async create(HORARIO_BLOQUEADO, PAUSA_CADA_PETICIONES, PAUSA_MINUTOS) {
        return await new CheckPerDayLastSentence(HORARIO_BLOQUEADO, PAUSA_CADA_PETICIONES, PAUSA_MINUTOS).processAllBases()
    }
    ...
    async check(BASE_NAME) {
        try {
            ...
            console.error("🚨 Error general en CheckPerDayLastSentence():", e.message);
            throw new Error("Error al ejecutar la función CheckPerDayLastSentence()");
        }
    };
}

module.exports = CheckPerDayLastSentence