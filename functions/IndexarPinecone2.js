...
class IndexarPinecone {
  constructor(doc, indexName, namespace, metadata) {
    this.doc = doc;
  ...
  static create(doc, indexName, namespace, metadata) {
    return new IndexarPinecone(doc, indexName, namespace, metadata).indexar();
  }
  ...
  async indexar() {
    ...
    try {
      ...
      const cunks = text_splitter.createDocuments([this.doc]);
