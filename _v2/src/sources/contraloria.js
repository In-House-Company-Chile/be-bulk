...
class ContraloriaSource extends BaseSource {
    ...
    _mapHit(hit) {
        ...
        // Usa documento_completo limpio (sin HTML) como fuente principal
        const text = [
            s.documento_completo || '',
        ...
        return {
            ...
            // Fuerza extensión .json en pipeline.js
            filename: `${hit._id}.json`,

            // Hit completo para guardar en content.raw en PostgreSQL
            // Estructura: { _id, _index, _type, _score, _source, highlight, sort }
            rawJson: hit,

            organism: (s['origen_'] || s.origenes || 'Contraloría General de la República').trim(),
    ...
    inferTema(doc) {
        ...
        if (t.includes('contrato') || t.includes('licitación') || t.includes('licitacion')) return 'contratacion_publica';
    ...
    static getTotalPages() { return 500; }
    static getParamsHelp() { return '--page=N (0–499)'; }
}
